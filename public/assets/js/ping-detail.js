...
(function () {
  var cfg = window.MONITORS_PING_DETAIL || {};
  ...
  var SM = window.Monitors || {};
  if (typeof SM.startPoller !== "function") return;

  function esc(value) {
    if (typeof SM.escapeHtml === "function") {
      return SM.escapeHtml(String(value ?? ""));
    }
  ...
  function fetchApply(range) {
    ...
    return fetch(url, { credentials: "same-origin", headers: { Accept: "application/json" } })
      ...
      .then((data) => {
        ...
        window.MONITORS_PING_HISTORY = Array.isArray(data.chart) ? data.chart : [];
        renderPingHistory();
        applySummary(data);

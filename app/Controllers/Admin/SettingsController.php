    ...
    public function index(Request $request): Response
    {
        ...
        try {
            if (metrics_is_partitioned()) {
                ...
                $dates = [];
                foreach ($parts as $p) {
                    $dataSize += (int) ($p['dlen'] ?? 0);
                    $indexSize += (int) ($p['ilen'] ?? 0);
                    $name = (string) ($p['pname'] ?? '');
                    if ($name === '' || $name === 'pmin' || $name === 'pmax') {
                        continue;
                    }
                ...
                $metricsStorage['data_size'] = $dataSize;
                $metricsStorage['index_size'] = $indexSize;
                $metricsStorage['table_size'] = $dataSize + $indexSize;
                $metricsStorage['partition_count'] = count($dates);
                if ($dates !== []) {
                    $metricsStorage['oldest_partition'] = min($dates);
                    $metricsStorage['newest_partition'] = max($dates);
                }
            } else {
                $table = db_one(
                ...
                if ($table) {
                    $metricsStorage['data_size'] = (int) ($table['data_length'] ?? 0);
                    $metricsStorage['index_size'] = (int) ($table['index_length'] ?? 0);
                    $metricsStorage['table_size'] = $metricsStorage['data_size'] + $metricsStorage['index_size'];
                }
            }
        } catch (\Throwable $e) {
            $metricsStorage['error'] = $e->getMessage();
        }

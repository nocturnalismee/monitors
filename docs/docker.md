...
#    Database host: db, port: 3306, name: servmon, user: servmon,
...
- Nightly DB backup (`01:15`, 30-day retention) lands in the
  `servmon_storage` volume under `storage/backups/`.
- `config/local.php` (written by the installer) lives in the
  `servmon_config` volume, so it survives `docker compose up --build`.
...
docker compose exec db mysql -uservmon -p servmon # db shell
